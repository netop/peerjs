...
},{"eventemitter3":"JJlS","./logger":"WOs9","./enums":"ZRYf"}],"TUPK":[function(require,module,exports) {
"use strict";
...
var CONNECTION_OPEN_TIMEOUT = 5000;
/**
 * An implementation of chained XHR polling.
 */

var HttpRequestLongPoll =
/** @class */
function () {
  function HttpRequestLongPoll(streamIndex, _httpUrl) {
    var _this = this;
    ...
    this._onError = function (sender) {
      sender;
    };

    this._onSuccess = function (sender) {
      sender;
    };
    ...
    this._xmlHttpRequest.onreadystatechange = function () {
      if (_this.needsClearPreviousRequest()) {
        _this.clearPreviousRequest();
      } else if (_this.isSuccess()) {
        _this._onSuccess(self);
      }
    };
  }

  Object.defineProperty(HttpRequestLongPoll.prototype, "onError", {
  ...
  Object.defineProperty(HttpRequestLongPoll.prototype, "onSuccess", {
    set: function set(handler) {
  ...
  HttpRequestLongPoll.prototype.send = function () {
    var _this = this;

    this._xmlHttpRequest.open("post", this._httpUrl + "/id?i=" + this.streamIndex, true);

    this._xmlHttpRequest.send(null);

    setTimeout(function () {
      if (_this._xmlHttpRequest.readyState < 2) {
        _this.abort();
  ...
  HttpRequestLongPoll.prototype.abort = function () {
    this._xmlHttpRequest.abort();
  };

  HttpRequestLongPoll.prototype.isSuccess = function () {
    return this._xmlHttpRequest.readyState > 2 && this._xmlHttpRequest.status === 200 && !!this._xmlHttpRequest.responseText;
  };

  HttpRequestLongPoll.prototype.needsClearPreviousRequest = function () {
    return this._xmlHttpRequest.readyState === 2 && !!this.previousRequest || !!this.previousRequest && this.previousRequest._xmlHttpRequest.readyState < 2;
  };

  HttpRequestLongPoll.prototype.clearPreviousRequest = function () {
    if (!this.previousRequest) return;
    this.previousRequest.abort();
    this.previousRequest = null;
  };

  HttpRequestLongPoll.prototype.getMessages = function () {
    return this._xmlHttpRequest.responseText.split("\n");
  };

  HttpRequestLongPoll.prototype.hasBufferedIndices = function () {
    return this.buffer.length > 0;
  };

  HttpRequestLongPoll.prototype.popBufferedIndex = function () {
    return this.buffer.shift();
  };

  HttpRequestLongPoll.prototype.pushIndexToBuffer = function (index) {
    this.buffer.push(index);
  };

  return HttpRequestLongPoll;
}();
/**
...
function (_super) {
  ...
  function HttpRequest(secure, host, port, path, key) {
    var _this = _super.call(this) || this;

    _this.HTTP_TIMEOUT = 25000; //ms

    _this._disconnected = true;
    _this._messagesQueue = [];
    var httpProtocol = secure ? "https://" : "http://";
    _this._baseUrl = httpProtocol + host + ":" + port + path + key;
    return _this;
  }

  HttpRequest.prototype.start = function (id, token) {
  ...
  HttpRequest.prototype._startXhrStream = function (streamIndex) {
    var _this = this;
    ...
    var newRequest = new HttpRequestLongPoll(streamIndex, this._httpUrl);
    ...
    newRequest.onSuccess = function () {
      _this._handleStream(newRequest);
    };
    ...
    try {
      ...
      logger_1.default.log("Error while sending through XMLHttpRequest", e.message);
    }
  };
  /** Handles onreadystatechange response as a stream. */


  HttpRequest.prototype._handleStream = function (httpRequest) {
    // 3 and 4 are loading/done state. All others are not relevant.
    var messages = httpRequest.getMessages(); // Check to see if anything needs to be processed on buffer.

    while (httpRequest.hasBufferedIndices()) {
      var index = httpRequest.popBufferedIndex();
      ...
      this.emit(enums_1.SocketEventType.Message, bufferedMessage);
    }

    var message = messages[httpRequest.index];

    if (message) {
      httpRequest.index += 1; // Buffering--this message is incomplete and we'll get to it next time.
      ...
      if (httpRequest.index === messages.length) {
        ...
        try {
          ...
          logger_1.default.log("Invalid server message", message);
          return;
        }

        this.emit(enums_1.SocketEventType.Message, message);
      }
    }
  };

  HttpRequest.prototype._setHTTPTimeout = function () {
    var _this = this;

    this._timeout = setTimeout(function () {
      var oldHttp = _this._httpRequest;

      _this._startXhrStream(oldHttp.streamIndex + 1);

      _this._httpRequest.previousRequest = oldHttp;
    }, this.HTTP_TIMEOUT);
  };
...
function (_super) {
  ...
  function ServerConnection(secure, host, port, path, key, pingInterval) {
    var _this = _super.call(this, null, null, null, null, null) || this;

    _this._webSocket = new socket_1.Socket(secure, host, port, path, key, pingInterval);
    _this._webSocket.emit = _this._interceptedSocketEmit.bind(_this);
    _this._httpRequest = new HttpRequest(secure, host, port, path, key);
    _this._httpRequest.emit = _this._interceptedHttpRequestEmit.bind(_this);
    return _this;
  }

  ServerConnection.prototype.start = function (id, token) {
    this._serverId = id;
  ...
  ServerConnection.prototype._startSocketOpenTimeout = function () {
    var _this = this;

    this._stopSocketOpenTimeout();

    this._socketOpenTimeout = setTimeout(function () {
      return _this._startHttp();
    }, CONNECTION_OPEN_TIMEOUT);
  };

  ServerConnection.prototype._stopSocketOpenTimeout = function () {
    clearTimeout(this._socketOpenTimeout);
    this._socketOpenTimeout = null;
  };

  ServerConnection.prototype._startSocket = function () {
    this._currentConnection = null;
  ...
  ServerConnection.prototype._startHttp = function () {
    this._currentConnection = null;
  ...
  ServerConnection.prototype._interceptedSocketEmit = function (event) {
    var args = [];
    ...
    if (this._socketOpenTimeout) {
      switch (event) {
        case enums_1.SocketEventType.Disconnected:
          return this._startHttp();

        case enums_1.SocketEventType.Message:
          this._stopSocketOpenTimeout();
    ...
    this._currentConnection === this._webSocket && this.emit.apply(this, __spread([event], args));
  };

  ServerConnection.prototype._interceptedHttpRequestEmit = function (event) {
    var args = [];
    ...
    this._currentConnection === this._httpRequest && this.emit.apply(this, __spread([event], args));
  };
...
},{"eventemitter3":"JJlS","./logger":"WOs9","./socket":"wJlv","./enums":"ZRYf"}],"HCdX":[function(require,module,exports) {
"use strict";
...
function (_super) {
  ...
  Peer.prototype._createServerConnection = function () {
    ...
    var socket = serverconnection_1.createServerConnection(this._options.secure, this._options.host, this._options.port, this._options.path, this._options.key, this._options.pingInterval, this._options.useXHRStreamFallback);
...
exports.Peer = Peer;
},{"eventemitter3":"JJlS","./util":"BHXf","./logger":"WOs9","./serverconnection":"TUPK","./mediaconnection":"dbHP","./dataconnection":"GBTQ","./enums":"ZRYf","./api":"in7L"}],"iTK6":[function(require,module,exports) {
"use strict";
